...
const { todos, uncheckedTodosCount } = storeToRefs(store);
const { removeTodo, toggleStatus } = store;
</script>
<template>
  <div class="container">
    <p v-if="uncheckedTodosCount.length === 0" class="title">Não há tarefas.</p>
    <p v-else class="title">
      Você possue {{ uncheckedTodosCount.length }} tarefas pendente(s).
    </p>
